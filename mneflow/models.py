...
"""
from layers import ConvDSV, ConvLayer, spatial_dropout, Dense, vgg_block, stack_layers
...
class Model(object):
    """Model object"""
    def __init__(self, h_params=None, params=None,model_path='',val_size=int(19*24), filt=None):
    ...
    def _buildGraph(self):
        ...
        elif self.h_params['architecture'] == 'lf-2':
            self.conv = stack_layers([ConvLayer(n_ls=self.params['n_ls'], 
                             filter_length=self.params['filter_length'],
                             pool=self.params['pooling'], 
                             nonlin_in=self.params['nonlin_in'],
                             nonlin_out=self.params['nonlin_hid'],
                             conv_type='lf'),
                             ConvLayer(n_ls=self.params['n_ls']//2, 
                             filter_length=self.params['filter_length']//2,
                             pool=self.params['pooling'], 
                             nonlin_in=self.params['nonlin_in'],
                             nonlin_out=self.params['nonlin_hid'],
                             conv_type='lf')])
