...
import gc
import keras_models
from keras_utils import plot_metrics, rmse, mse, r_square, soft_acc, _speak
    ...
    for x, y in dataset.train.shuffle(train_elems).take(train_elems):
        ...
        for kk in range(k):
            for s in range(nseq):
                ...
                # Track progress
                t_metrics.append([rmse(yn, y_), mse(yn, y_), r_square(yn, y_),
                                  soft_acc(yn, y_), cost, yn, y_])
            ...
            for vx, vy in dataset.val.shuffle(val_elems).take(val_elems):
                ...
                for vs in range(vx.shape[2]):
                    ...
                    # Track progress
                    v_metrics.append([rmse(yn, y_), mse(yn, y_), r_square(yn, y_),
                                      soft_acc(yn, y_), vcost, yn, y_])
...
for vx, vy in test_dataset.shuffle(test_elems).take(test_elems):
    ...
    for vs in range(vx.shape[2]):
        ...
        # Track progress
        test_metrics.append([rmse(yn, y_), mse(yn, y_), r_square(yn, y_),
                             soft_acc(yn, y_), vcost, yn, y_])
