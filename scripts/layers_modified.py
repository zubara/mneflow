...
class LFTConv():
    ...
    def __init__(self, scope="lf-conv", n_ls=32,  nonlin_out=tf.nn.relu,
                 filter_length=7, stride=1, pooling=2, padding='SAME', conv_type='2d'):
        self.scope = scope
        self.size = n_ls
...
class DeMixing():
    ...
    """
    def __init__(self, scope="de-mix", n_ls=32,  nonlin=tf.identity, expand=True):
        self.scope = scope
        self.size = n_ls
        self.nonlin = nonlin
        self.expand = expand
            ...
            while True:
                try:
                    ...
                    if self.conv_type == 'depthwise':
                        ...
                        conv_ = self.nonlin(tf.nn.conv3d(x, self.filters,
                                            strides=[1, 1, self.stride, self.stride, 1],
                                            padding=self.padding) + self.b)



                    if self.conv_type == '3d':
                        conv_ = tf.nn.max_pool3d(conv_, ksize=[1, self.pool, 1, 1, 1],
                                           strides=[1, 1, 1, 1, 1],
                                           padding='SAME')
                    else:
                    ...
                    if self.domain == 'time':
                        ...
                        if self.conv_type == '3d':
                            ...
                            self.filters = weight_variable([1, self.filter_length[0], self.filter_length[1],
                                                       self.inch, self.size],
                                                       name='weights')
